...
def add_to_cart(request, food_id):
    if request.user.is_authenticated:
        # Modern AJAX check (Django 4.0+ compatible)
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            # Check if the food item exists
            try:
                fooditem = FoodItem.objects.get(id=food_id)
                # Check if the user already has this food item in their cart
                try:
...
def decrease_cart(request, food_id):
    if request.user.is_authenticated:
        ...
        return JsonResponse({
            'status': 'login_required', 
            'message': 'Please login to modify items in cart.'
        })
...
def search(request):
    if not 'address' in request.GET:
        ...
        if latitude and longitude and radius:
            ...
            vendors =  Vendor.objects.filter(Q(vendor_name__icontains=keyword) | 
            Q(id__in=fetch_vendors_by_fooditems),
            is_approved=True, 
            user__is_active=True, 
            user_profile__location__distance_lte=(pnt, 
            D(km=radius))).annotate(
